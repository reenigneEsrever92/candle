...
impl WgpuBackend {
    pub fn ones(&self, amount: u64) -> WgpuResult<Vec<f32>> {
        self.run_kernel("ones", 1, amount, 0.0, 0.0)
    ...
    pub fn rand_uniform_f32(
        ...
        self.run_kernel("rand_uniform_f32", seed, amount, min, max)
            .map(|bytes| bytemuck::cast_slice(&bytes).to_vec())
    }
...
pub mod tests {
    ...
    fn test_fill_ones() {
        ...
        let rand = backend.ones(512).unwrap();
    ...
    fn test_fill_random() {
        ...
        // meam must approach zero given a large enough sample size
        assert!(mean >= -1.0);
        assert!(mean <= 1.0);
    }
}
